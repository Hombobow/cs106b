...
/*
 * the enqueue function takes in a DataPoint elem and adds that to our priority
 * queue _elements in the order of priority within elem
 */
void PQArray::enqueue(DataPoint elem) {
    ...
    //store the elements priority
    double elemP = elem.priority;

    //place elem at the end of the array
    _elements[size()] = elem;

    //loop through each value and shift it to the left
    //if elem's priority is greater than it
    for (int i = size() - 1; i >= 0; i--) {
        if (elemP > _elements[i].priority) {
        ...
        //if we moved every single one without adding elem, add elem
        if (i == 0) {
            _elements[0] = elem;
        }
    }

    //update our _numFilled count
    _numFilled++;
}
...
/* This function doubles the size of our priority queue by
 * creating a new pointer at a new location in memory with double the size and
 * copying over the previous locations cells to the new location then pointing
 * our old pointer at the new location
 */
void PQArray::enlargeArray() {
    //request space for double the amount of elements
    DataPoint* _newElements = new DataPoint[_numAllocated * 2]();

    //copy the old data to the new priority queue
    for (int i = 0; i < size(); i++) {
        _newElements[i] = _elements[i];
    }

    //delete the old memory location
    delete[] _elements;

    //point our previous pointer to the new memory location
    _elements = _newElements;


    //double the integer reflecting number of allocated memory slots
    _numAllocated *= 2;
}

/* * * * * * Test Cases Below This Point * * * * * */

//letting the program know about the fillQueue and emptyQueue functions
void fillQueue(PQArray& pq, int n);
void emptyQueue(PQArray& pq, int n);

STUDENT_TEST("PQArray timing test, fillQueue and emptyQueue, over 4 input sizes") {
    PQArray pq;

    //timing tests for 4 different input sizes of fillQueue, doubling each time
    for (int n = 20000; n <= 160000; n *= 2) {
        TIME_OPERATION(n, fillQueue(pq, n));
        cout << "done" << endl;
    }

    //timing tests for 4 different input sizes of emptyQueue, doubling each time
    for (int n = 40000; n <= 320000; n *= 2) {
        fillQueue(pq, n);
